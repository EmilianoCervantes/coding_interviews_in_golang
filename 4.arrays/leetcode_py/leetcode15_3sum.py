...
class SolutionDeprecated:
    ...
    It exceeds the time limit for the test case 310.
    What else can we do?
    """

    def __twoSum(self, nums: list[int], target: int, origin_position: int) -> list[list[int]]:
        """
        Returns the 2 numbers
        """
        numbers_so_far: dict[int, int] = {}
        pairs_to_result: list[list[int]] = []
        pairs_so_far: dict[str, str] = {}
        for i, num in enumerate(nums):
            if i != origin_position:
                """
                We don't want to count the same number twice as per:
                i != j, i != k, and j != k 
                """
                look_up = target - num
    ...
    def threeSum(self, nums: list[int]) -> list[list[int]]:
        ...
        for position, num in enumerate(nums):
            ...
            for pair in found_pairs:
                # Let's order the found numbers from smallest to largest
                triplet = [num, *pair]
                triplet.sort()

                triplet_as_string = f"{triplet}"
...
# Note: I did this solution based on feedback from other solutions at leetcode, this is NOT mine.
class Solution:
    def threeSum(self, nums: list[int]) -> list[list[int]]:
        """
        Let's rethink the 2 sum problem.
        We need to look at our other saved numbers like a mirror.
        For that we will sort or separate the numbers (whatever you prefer).
        This is the O(n) solution.
        """
        if len(nums) < 3:
            return []
        ...
        for num in nums:
            if num > 0:
                ...
                # Small optimization - Help with memory
        ...
        if len(zeroes) > 0:
            ...
            # Picking either will show us the mirror pairs either way
            for positive in positives:
                negative_target = -1 * positive
                if negative_target in negatives_set:
                    results.add((negative_target, 0, positive))

        # Now let's check taking 2 from negatives or taking 2 from positives
        # and finding the value on the other side

        for pointer1, positive1 in enumerate(positives):
            for pointer2 in range(pointer1+1, len(positives)):
        ...
        for pointer1, negative1 in enumerate(negatives):
            for pointer2 in range(pointer1+1, len(negatives)):
