...
func search(nums []int, target int) int {
	...
	isLessThanMiddle := target < nums[middlePointer]

	// Less than the right position
	if target < rightValue {
		// Between left and middle
		if isLessThanMiddle {
			distanceLeft := int(math.Abs(float64(nums[leftPointer] + target)))
			distanceMiddle := int(math.Abs(float64(nums[middlePointer] - target)))

			if distanceLeft < distanceMiddle {
				leftPointer = leftPointer + distanceLeft
				for leftPointer < middlePointer {
					leftPointer += 1
		...
		rightPointer = rightPointer - int(math.Abs(float64(rightValue-target)))
	}
